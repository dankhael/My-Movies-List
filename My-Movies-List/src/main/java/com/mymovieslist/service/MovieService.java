...
public class MovieService {
    ...
    public List<Movie> findMoviesByName(String name) {
        return movieRepo.findMoviesByNameContains(name)
                .orElseThrow(() -> new MovieNotFoundException("Movies by Name" + name + "were not found"));
    }
    ...
    public List<Movie> findMoviesByFavorite(Boolean favorite) {
        String errorMessage;
        if (favorite) {
            errorMessage = "No favorite movie found.";
        } else {
            errorMessage = "No unfavorite movie found.";
        }
        return movieRepo.findMoviesByFavorite(favorite).orElseThrow(() -> new MovieNotFoundException(errorMessage));
    }
